...
export function PostCard({ post }) {
    ...
    const handleDelete = (id) => {
        toast((t) => (
            <div>
                <p className='text-white'>Do you want to delete? <h2 className='text-center'>{post.title}</h2></p>
                <div>
        ...
        >
            ...
            <div className="px-4 py-7">
                <div className="flex justify-between">
                    <h3>{post.title}</h3>
                    <button className="bg-red-600 hover:bg-red-500 px-2 py-1 rounded-sm text-sm"
                        onClick={(e) => {
                            e.stopPropagation()
            ...
            {post.image && <img src={post.image.url} alt="" className='w-full object-cover' />}
